...
import net.codingwell.scalaguice.{ ScalaModule, ScalaMapBinder }

class ScalaMapBinderSpec extends WordSpec with MustMatchers {
  ...
  private def validate[K, V](map: im.Map[K, V], expected: (K, V)*): Unit = {
    map must have size expected.length
    for ((k, v) <- expected) {
      map(k) must equal(v)
    }
  }
