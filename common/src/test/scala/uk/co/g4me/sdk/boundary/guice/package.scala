...
package uk.co.g4me.sdk.boundary
...
package object guice {
  ...
  private[guice] def typeOf[T](implicit m: Manifest[T]): Type = {
   ...
   */
  private[guice] def nameOf[T](key: Key[T]): String = {
  ...
  private[guice]type HKClassTag[CC2[_]] = ClassTag[CC2[_]]

  private[guice] class WrapHelper[WType[_]: HKClassTag] {
  ...
  private[guice] def wrap[WType[_]: HKClassTag] = new WrapHelper[WType]

  /** Helpers to wrap K, V into some enclosing type that takes two type parameters. */
  private[guice]type HKClassTag2[CC2[_, _]] = ClassTag[CC2[_, _]]

  private[guice] class WrapHelper2[WType[_, _]: HKClassTag2] {
  ...
  private[guice] def wrap2[WType[_, _]: HKClassTag2] = new WrapHelper2[WType]
