...
package uk.co.g4me.sdk.common.util
...
object ServiceLoader {
   ...
   */
  def load[S](service: Class[S], loader: ClassLoader, config: Configuration = Configuration.empty): ServiceLoader[S] = {
    new ServiceLoader[S](service, loader, config)
  }
...
final class ServiceLoader[S](svc: Class[S], cl: ClassLoader, config: Configuration) extends java.lang.Iterable[S] {
  ...
  private class LazyIterator(service: Class[S], loader: ClassLoader) extends Iterator[S] {
    ...
    override def next: S = {
      ...
      try {
        //construct here
        val c = Class.forName(cn, true, loader).getDeclaredConstructor(classOf[Configuration]).newInstance(Configuration.empty)
        val p = service.cast(c)
        providers.put(cn, p)
        return p
      } catch {
